...
import { BookOpen, Search, Heart, Clock, Grid, List, Star, Eye } from 'lucide-react'
...
import {
  ...
  searchVadeMecumImproved,
} from '../../services/vadeMecumServiceImproved'
...
const VadeMecumImproved = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      let result
      // Usa o novo serviço melhorado que busca em múltiplos campos
      result = await searchVadeMecumImproved(
      ...
      if (result && result.success) {
        ...
        // Log para debug
        console.log(`Busca por "${debouncedSearchTerm}" encontrou ${result.totalFound || result.documents.length} documentos`)
      }
    } catch (error) {
  ...
  const handleLoadMore = async () => {
    ...
    try {
      let result
      // Usa o novo serviço melhorado para carregar mais documentos
      result = await searchVadeMecumImproved(
        debouncedSearchTerm.trim() || null,
        documentsPerPage,
        lastDocument,
